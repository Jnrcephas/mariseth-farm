...
import SmallholderSearch from "./SmallholderSearch";
...
export default function SmallholderFarmers(){
    ...
    const columns: ColumnDef<any>[] = [
        ...
        { header: "Land Ownership", accessorKey: "land_ownership",
            cell: (_rowData) => {
                ...
                return (
                  <div className="">
                    {rowData?.farm?.land_ownership &&
                    <Badge variant={statusBadgeMap[rowData?.farm?.land_ownership?.toLowerCase()]} className="capitalize">
                        {rowData?.farm?.land_ownership}
                    </Badge>}
                  </div>
                );
                  ...
                  <div className="">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                            ...
                            {update_farmer && 
                                <DropdownMenuItem className="cursor-pointer" onClick={() => router.push(`${routeTo.editSmallholderFarmer}/${tableData.row?.original?.id}`)}>Edit </DropdownMenuItem>
                            }
                            {delete_farmer && 
    ...
    <div className="mt-3">
        <CustomTable 
            searchFilter={<SmallholderSearch setFilters={setFilters} filters={filters} refetch={refetch} isLoading={isLoading} />}
            columns={columns} 
            data={data?.results || []} 
